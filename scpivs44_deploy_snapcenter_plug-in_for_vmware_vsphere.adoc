...
* vVol データストア上の VM を保護するには、まずONTAP tools for VMware vSphereを導入します。サポートされているONTAPツールのバージョンについては、 https://imt.netapp.com/matrix/imt.jsp?components=134348;&solution=1517&isHWU&src=IMT["NetApp Interoperability Matrix Tool"^] 。  ONTAPツールは、 ONTAPおよび VMware Web クライアント上のストレージをプロビジョニングおよび構成します。
